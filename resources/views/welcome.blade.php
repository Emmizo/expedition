        ...
        <div class="carousel-inner">
            @foreach($sliders as $slider)
                <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                    <div class="d-flex align-items-center justify-content-center" style="min-height: 400px; background: #184C55;">
                        ...
                        <div class="carousel-caption d-none d-md-block text-start">
                            <h2 class="fw-bold text-white">{{ $slider->title }}</h2>
                            @if($slider->description)
                                <p class="lead text-white">{{ $slider->description }}</p>
                            @endif
                            @if($slider->button_link && $slider->button_text)
                                <a href="{{ $slider->button_link }}" class="btn btn-brand btn-lg">{{ $slider->button_text }}</a>
                            @endif
                        </div>
                    </div>
                </div>
...
<!-- Hero Section -->
<section class="position-relative" style="background: url('/assets/images/hero.jpg') center/cover no-repeat; min-height: 70vh;">
    <div class="container hero-overlay">
        <div class="row align-items-center">
            <div class="col-lg-7 mb-4 mb-lg-0">
                <h1 class="display-4 fw-bold mb-3 text-white">Driving <span class="text-accent">Ideas</span>,<br>Teamwork & <span class="text-accent">Future-Ready</span><br>Solutions</h1>
                <p class="lead mb-4 text-white">Tech areas, services, and solutions related to content and testing. Here are some words related to content and testing. Hindenburg's report includes a text confirming that the section of road shown in the video could accelerate a crossing vehicle to highway speeds with text messages from a former Nokia employee agreeing to confirm the facts.</p>
                <a href="#" class="btn btn-brand btn-lg">Enable Innovation</a>
            </div>
        </div>
    </div>
</section>
...
<section class="bg-white py-3 border-bottom">
    <div class="container text-center">
        <span class="fw-semibold text-brand">Here are some words related to content and testing:</span>
        ...
        <a href="#" class="btn btn-outline-brand btn-sm ms-3">Let's Get Started</a>
    </div>
</section>
...
<section class="py-5 bg-light">
    <div class="container">
        <h2 class="section-title text-center">Innovative Tech Solutions for Your Business and Personal Growth</h2>
            ...
            @foreach($services as $service)
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        <div class="mb-3 mt-3">
                            @if($service->icon)
                                <i class="{{ $service->icon }} display-4 text-brand"></i>
                            @endif
                        </div>
...
<section class="bg-white py-4 border-top border-bottom">
    <div class="container text-center">
        <span class="fw-semibold text-brand">Tell us more about your needs.</span>
        <span class="text-muted ms-2">We're here to listen and provide the solutions that address your legal concerns.</span>
        <a href="#" class="btn btn-brand btn-sm ms-3">Inform us</a>
    </div>
</section>
...
<section class="py-5 bg-brand text-white">
    <div class="container">
        <h2 class="text-center mb-4 section-title text-white">Why TechVerse</h2>
        <div class="row text-center">
            @foreach($whyChooseUsItems as $item)
                <div class="col-md-4 mb-4">
                    <div class="card h-100 bg-brand text-white border-0 shadow-sm">
                        <div class="card-body">
                            @if($item->icon)
                                <div class="mb-3"><i class="{{ $item->icon }} fa-2x text-accent"></i></div>
            @endif
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-6 mb-4 mb-md-0">
                <h5 class="text-brand">What we Do</h5>
            ...
            <div class="col-md-3 mb-4 mb-md-0">
                <h5 class="text-brand">Our Mission</h5>
            ...
            <div class="col-md-3">
                <h5 class="text-brand">Our Vision</h5>
                ...
                <div class="col-md-4 mb-4"></div>
                    <div class="card h-100 shadow-sm border-0">
                        <div class="card-body">
                            ...
                            <footer class="blockquote-footer text-end text-brand">{{ $testimonial->author }}</footer>
                        </div>
                    </div>
                </div>
            @endforeach
                ...
                <div class="col-md-5 mb-4 mb-md-0">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body text-center">
                            <img src="{{ $member->image_path ? asset('storage/' . $member->image_path) : 'https://ui-avatars.com/api/?name=' . urlencode($member->name) . '&background=184C55&color=fff&size=100' }}" alt="{{ $member->name }}" class="rounded-circle mb-3" width="100" height="100">
                            <h5 class="fw-bold mb-1 text-brand">{{ $member->name }}</h5>
                            <p class="text-muted mb-2">{{ $member->title }}</p>
                            <p class="mb-2">{{ $member->bio }}</p>
                            @if($member->linkedin_url)
                                <a href="{{ $member->linkedin_url }}" class="text-brand" target="_blank"><i class="bi bi-linkedin"></i> Visit my LinkedIn profile</a>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <div class="container">
        ...
        @endif
                        <div class="card-body d-flex flex-column">
                            ...
                            <a href="{{ route('blog.show', $post) }}" class="btn btn-outline-brand mt-auto">Read More</a>
                        </div>
                    </div>
                </div>
            @empty
        ...
        <div class="text-center mt-4">
            <a href="{{ route('blog.index') }}" class="btn btn-brand">View All Blog Posts</a>
        </div>
    </div>
</section>
...
<section class="py-5 bg-light">
    <div class="container">
        <h2 class="section-title text-center">Contact TechVerse</h2>
